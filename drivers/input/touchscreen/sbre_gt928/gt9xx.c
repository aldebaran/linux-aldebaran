...
static void gtp_pen_init(struct goodix_ts_data *ts)
{
	...
	/*input_set_abs_params(ts->pen_dev, ABS_MT_POSITION_X, 0, 600, 0, 0);*/
	input_set_abs_params(ts->pen_dev, ABS_MT_POSITION_X, 0, ts->abs_x_max, 0, 0);
	/*input_set_abs_params(ts->pen_dev, ABS_MT_POSITION_Y, 0, 1024, 0, 0);*/
...
void gtp_reset_guitar(struct i2c_client *client, s32 ms)
{
	...
	GTP_GPIO_OUTPUT(gtp_rst_gpio, 1);
	 ...
	 /* According to the datasheet 2014 the delay is 55ms so we have added 60ms */
	msleep(60);                          // T4: > 5ms

	//GTP_GPIO_OUTPUT(gtp_ctrl_gpio, 1);
	GTP_GPIO_AS_INT(gtp_int_gpio);    // end select I2C slave addr
...
/*	ret = GTP_GPIO_REQUEST(gtp_ctrl_gpio, "GTP CTRL PORT");
        if (ret < 0)
        {
                GTP_ERROR("Failed to request GPIO:%d, ERRNO:%d",(s32)gtp_ctrl_gpio,ret);
                ret = -ENODEV;
        }*/
...
static s8 gtp_request_input_dev(struct goodix_ts_data *ts)
{
	...
	/*input_set_abs_params(ts->input_dev, ABS_MT_POSITION_X, 0, 600, 0, 0);*/
	input_set_abs_params(ts->input_dev, ABS_MT_POSITION_X, 0, ts->abs_x_max, 0, 0);
	/*input_set_abs_params(ts->input_dev, ABS_MT_POSITION_Y, 0, 1024, 0, 0);*/
...
static void gtp_parse_dt(struct device *dev)
{
	...
	gtp_ctrl_gpio = of_get_named_gpio(np, "goodix,ctrl-gpio", 0);
...
int gtp_parse_dt_cfg(struct device *dev, u8 *cfg, int *cfg_len, u8 sid)
{
	...
	if (!prop || !prop->value || *cfg_len == 0 || *cfg_len > GTP_CONFIG_MAX_LENGTH) {
		...
		//GTP_ERROR("\n\nNAG:prop %d, prop->value %d, *cfg_len %d, \n\n", prop, prop->value, *cfg_len);
