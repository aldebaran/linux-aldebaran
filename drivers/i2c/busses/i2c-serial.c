 ...
 * */
static inline char* i2c_serial_get_version(struct i2c_adapter *adapter) {
  ...
  // no need to check : if things went wrong, buffer is empty anyway...
  return i2cd->buffer;
...
static int i2c_connect(struct serio *serio, struct serio_driver *drv)
{
  ...
  if (err) {
    I2CS_DEBUG("adapter registration failed.");
    goto exit_close;
  }

  I2CS_INFO("I2C over serial port registered.");

  I2CS_INFO("PSoC firmware version '%s'.", i2c_serial_get_version(adapter));
