				...
				int num)
{
	...
	for (i = 0; i < num; i++) {
		if (!(msgs[i].flags & I2C_M_RD))
			rd_lmax = max((ulong) rd_lmax, (ulong) msgs[i].len);
		else
			wr_lmax = max((ulong) wr_lmax, (ulong) msgs[i].len);
	}
	rd_lmax += rd_hdr_len;
	wr_lmax += wr_hdr_len;
	...
	for (i = 0; i < num; ++i) {
		...
		memset(rd_buf, 0, max(rd_lmax, (ulong) CGBUF_LEN));
		memset(wr_buf, 0, max(wr_lmax, (ulong) CGBUF_LEN));
		...
		if (msgs[i].flags & I2C_M_RD) {
			ulong rd_len = rd_hdr->response_len - rd_hdr_len;
			ulong cpy_len =
			    min((ulong) msgs[i].len, (ulong) rd_len);
...
static inline int cgos_i2c_adapter_create(ulong handle, ulong interface)
{
	int r;
	struct cgos_i2c_adapter *dev;
